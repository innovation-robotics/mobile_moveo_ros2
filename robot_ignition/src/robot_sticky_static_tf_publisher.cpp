 ...
 *
 */
class RobotStaticTfPublisher : public rclcpp::Node
{
public:
  RobotStaticTfPublisher() : Node("tf_broadcaster")
  {
    // Publish every 10ms -> 100 Hz
    timer_ = this->create_wall_timer(
      10ms, std::bind(&RobotStaticTfPublisher::timer_callback, this));
  }
...
int main(int argc, char* argv[])
{
  ...
  rclcpp::spin(std::make_shared<RobotStaticTfPublisher>());
  rclcpp::shutdown();
  return 0;
}
